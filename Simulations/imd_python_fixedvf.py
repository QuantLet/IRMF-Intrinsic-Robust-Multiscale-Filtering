...
class ProgressReporter:
    def __init__(self, every=10000, total=None, prefix=""):
    ...
    def step(self, n=1):
        ...
        if self.count % self.every == 0:
            if self.total is None:
                print(f"{self.prefix}completed {self.count}", flush=True)
            else:
                print(f"{self.prefix}completed {self.count}/{self.total}", flush=True)


def weighted_minimizer(target_grid, obs_grid, objective_at_a, bandwidth, lower, upper, progress=None):
...
def imd_decompose_with_linearization(
    ...
    progress = ProgressReporter(
        ...
        total=K_steps * n_time * (n_paths + 2),
...
def build_experiment(
    ...
    res = imd_decompose_with_linearization(
        ...
        progress_prefix=f"c={case_value}, t={type_value}, tc={tc_value}:",
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the IMD simulation translated from combine2.R and save five diagnostic plots."
    )
    parser.add_argument("--c", "--case", dest="case_value", type=int, default=1, choices=[1, 2])
    ...
    parser.add_argument("--progress-every", dest="progress_every", type=int, default=10000)
    ...
    if args.progress_every < 0:
        parser.error("--progress-every must be 0 or a positive integer.")
    return args
